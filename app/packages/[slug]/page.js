...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    return {
      ...
      description: pkg.description?.substring(0, 150) || "سفارش سایت اختصاصی با طراحی منحصر به فرد و ارزان قیمت و پشتیبانی 24 ساعته",
      openGraph: {
        images: [pkg.image]
      }
    };
        ...
        <main className="bg-linear-to-b flex flex-col justify-center items-center min-h-screen from-[#001122] to-[#002233]">
          <div className="px-6 py-16 max-w-7xl mx-auto md:mx-0 text-gray-200 flex flex-col lg:flex-row justify-around items-center">
            ...
            <div className="w-[70%] lg:mt-0 mt-8 mr-4 md:mr-40">
              <h1 className="hidden md:block text-4xl font-bold mb-6">{pkg.title}</h1>
              <p className="text-xl mb-4 underline ">{pkg.price ==="0" ? "برای خرید تماس بگیرید ." :formatPrice(pkg.price)}</p>
              ...
              <div className=" flex justify-start items-center mt-8 gap-3 md:gap-8">
                {pkg.price === "0" ? <p className="bg-[#0066ff] hover:bg-[#0251c7] shadow-lg hover:shadow-[#0066ff]/50 py-2 px-4 rounded-2xl">برای خرید تماس بگیرید .</p> :(
